...
public class TransactionsDAO extends AbstractDAO {
     ...
     *
     * @param page     The page number if paginated
     * @param pageSize The size of each page if paginated
     * @param order    The order clause
     * @param filter   Used to filter the results
    ...
    public int count(TransactionFilter filter) throws SQLException {
     ...
     *
     * @param precision A number between 1 and the count of transactions, will default to
    ...
    public int sumAll() throws SQLException {
        return sumAll(new TransactionFilter());
    }

    public int sumAll(TransactionFilter filter) throws SQLException {
        String sql = SQLUtils.getQuery("Transactions.SumAll").replace("{filter}", filter.getFilterClause());
        try (Connection conn = SQLUtils.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql);
             ResultSet rs = ps.executeQuery()) {
            if (rs.next()) {
